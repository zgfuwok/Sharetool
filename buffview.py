...
xnhac = "\033[1;36m"
# Đánh Dấu Bản Quyền
vchh_tool = trang + "" + do + "[" + trang + "=.=" + do + "] " + trang + "=> "
vchh = trang + "" + do + "[" + trang + "=.=" + do + "] " + trang + "=> "
# Config
__ZALO__ = '0326023566'
__ADMIN__ = 'Vũ Công Huy Hoàng'
...
banner()
# VÀO GIAO DIỆN
cookie = input(vchh_tool+luc+'Vui Lòng Nhập Cookie Facebook Chứa Page Pro5'+trang+': '+vang)
headers = {
...
try:
    print(vchh_tool+xnhac+"Đang Check Live Cookie...", end="\r")
    ...
    exit(vchh_tool+do+"Cookie Die Vui Lòng Kiểm Tra Lại!!!")
clear()
banner()
...
print(vchh_tool+luc+'Đã Tìm Thấy '+trang+str(len(list_id_name_page))+luc+' Page Pro5')
thanh()
url_str = input(vchh_tool+luc+'Vui Lòng Nhập Link Str Cần Tăng View'+trang+': '+vang)
# GET DỮ LIỆU TRONG URL
thanhphan9 = url_str.split('/')[5].split('/?')[0]
# NHẬP ĐÓ VIEW BẠN MUỐN TĂNG
thanh()
soluongview = int(input(vchh_tool+luc+'Vui Lòng Số View Bạn Cần Tăng'+trang+': '+vang))
thanh()
delay = int(input(vchh_tool+luc+'Vui Lòng Nhập Delay View'+trang+': '+vang))
thanh()
# RUN CODE
